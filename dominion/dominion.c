...
int cardEffectMinion(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	/*This function is refactored for Assignemnt 2*/
	/*var card, choice3, and bonus are not used but passed for consistency*/
	...
	else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
	{
		...
		//draw 4
		for (i = 0; i <= 4; i++)		/*Minion Bug #1, changed '<' to '<=' to effectivly make the current player draw an additional card*/
		{
		...
		for (i = 0; i < state->numPlayers; i++)
		{
			if (i != currentPlayer)
			{
				if ( state->handCount[i] > 4 )
				{
					...
					for (j = 0; j < 4; j++)
					{
						//drawCard(i, state); 	 /*Minion Bug #2, commented out the function call that has opponents draw cards*/
					}
				}
			}
		}
...
int cardEffectAmbassador(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	/*This function is refactored for Assignemnt 2*/
	/*var card, choice3, and bonus are not used but passed for consistency*/
	
	int i;
	int j = 1;		//used to check if player has enough cards to discard		/*Ambassador Bug #1, changed initilization of j to '1' instead of '0'*/
	int currentPlayer = whoseTurn(state);
	...
	for (j = 0; j < choice2; j++)
	{
		for (i = 0; i < state->handCount[currentPlayer]; i++)
		{
			if (state->hand[currentPlayer][i] == state->hand[currentPlayer][choice1])
			{
				...
				//break;			/*Ambassador Bug #2, commented out the break statement which in the right circumstances could cause the program to execute too many loops*/
			}
		}
	}
...
int cardEffectTribute(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	/*This function is refactored for Assignemnt 2*/
...
int cardEffectMine(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	/*var card, choice3, and bonus are not used but passed for consistency*/
	int currentPlayer = whoseTurn(state);
	int j = state->hand[currentPlayer][choice2];  //store card we will trash		/*Mine Bug #1, changed 'choice1' to 'choice2', this will cause undefined behavior*/
	...
	{
		return 0;			/*Mine Bug #2, changed the return value from '-1' to '0' to effectively pass bad information back to the calling function*/
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
		return cardEffectMine(card, choice1, choice2, choice3, state, handPos, bonus);
    ...
    case baron:
		irrVal = cardEffectBaron(card, choice1, choice2, choice3, state, handPos, bonus);
        return 0;
    ...
    case minion:
		irrVal = cardEffectMinion(card, choice1, choice2, choice3, state, handPos, bonus);
        return 0;
    ...
    case tribute:
		irrVal = cardEffectTribute(card, choice1, choice2, choice3, state, handPos, bonus);
        return 0;


    case ambassador:
		return cardEffectAmbassador(card, choice1, choice2, choice3, state, handPos, bonus);
